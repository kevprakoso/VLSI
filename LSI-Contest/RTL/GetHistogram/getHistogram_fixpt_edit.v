...
module getHistogram_fixpt
  ...
  wire signed [13:0] delayMatch_out [0:8];  // sfix14_E1  
  ...
  reg signed [13:0] delayMatch_1_reg [0:8] [0:63];  // sfix14 [64]
  ...
  wire signed [13:0] delayMatch_1_reg_next [0:8][0:63];  // sfix14_E1 [64]
  ...
  reg signed [13:0] angles_1_regout [0:63];  // sfix14_En11
  ...
  reg [13:0] magnitudes_regout [0:63];  // ufix14_En7
  ...
  reg signed [31:0] p5_t [0:12];  // int32
  ...
  reg signed [43:0] p5_div_temp [0:18];  // sfix44_En24
  ...
  reg signed [16:0] p5_sub_cast_0;  // sfix17_En
  ...
  reg signed [56:0] p5_sub_cast_6;  // sfix57_En3
  ...
  reg signed [18:0] p5_sub_cast_20;  // sfix19_En1
  ...
  getHistogram_fixpt_enb_bypass u_getHistogram_fixpt_enb_bypass (
  .clk_1(clk),
  .reset_1(reset),
  ...
  always @(posedge clk or posedge reset)
    begin : c
      if (reset) begin
        clk_enable_regout [0] <= 1'b0;
      end
      else begin
        clk_enable_regout [0] <= clk_enable_1;
      end
    end
  ...
  always @(posedge clk or posedge reset)
    begin : c_1
	  for (i = 63; i > 0; i = i-1 )
	    begin
		  if (reset) begin
			clk_enable_regout [i] <= 1'b0;
		  end
		  else begin
			clk_enable_regout [i] <= clk_enable_regout [i-1];
		end
	  end
    end
    ...
    begin : delayMatch_1_process
      if (reset == 1'b1) begin
        ...
        begin
          for (k = 0; k < 64; k = k + 1)
          begin
            delayMatch_1_reg[j][k] <= 14'sb00000000000000;
          end
        end
      end
      else begin
        if (enb) begin
          for (j = 0; j < 9; j = j + 1)
            begin
              for (k = 0; k < 64; k = k + 1)
              begin
                delayMatch_1_reg[j][k] <= delayMatch_1_reg_next[j][k];
              end
          end
        end
      end
